...
RUN microdnf install -y --nodocs redis --enablerepo=centos8-appstream && dnf clean all
RUN microdnf install -y --nodocs hostname procps-ng iproute net-tools ethtool nmap iputils https://dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm && dnf clean all
